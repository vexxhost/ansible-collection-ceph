...
# ---------------------------------------------------------------------------
# Vendored from:
#   https://github.com/ceph/ceph-ansible/blob/8599b192d33e1c55104c1c2fd1abfa8431c64664/library/ceph_ec_profile.py
...
#   - added the remaining valid (non-deprecated) squid/tentacle profile keys:
#     w, packetsize, jerasure-per-chunk-alignment (bool), crush-locality,
#     mapping, layers, crush-steps, crush-num-failure-domains,
#     crush-osds-per-failure-domain, directory.
#   - jerasure_per_chunk_alignment is a real bool, emitted as lowercase
#     true/false for ceph.
#   - idempotency compare is now subset-based (desired keys must match; ceph's
#     echoed defaults are ignored) instead of exact dict equality.
...
from ansible_collections.vexxhost.ceph.plugins.module_utils.common import is_containerized, generate_cmd, exec_command, exit_module
import datetime
import json
...
def parse_user_profile(module):
    # Exact ceph erasure-code-profile key spellings (hyphen vs underscore
    # matters). The ansible param is the same with hyphens -> underscores.
    profile_keys = ['plugin',
                    'k', 'm', 'd', 'l', 'c',
                    'w', 'packetsize', 'jerasure-per-chunk-alignment',
                    'stripe_unit', 'scalar_mds', 'technique',
                    'crush-locality', 'mapping', 'layers', 'crush-steps',
    ...
    for key in profile_keys:
        ...
        # ceph wants lowercase true/false for boolean keys, not Python True/False.
...
def profile_satisfied(desired, current):
    '''
    True if every desired key/value is already present and equal in the current
    profile. `current` (from `erasure-code-profile get`) also carries keys ceph
    fills in with defaults (e.g. technique, crush-*), which we ignore — so an
    already-applied profile does not read back as changed.
    '''
    return all(k in current and current[k] == v for k, v in desired.items())
...
def run_module():
    module_args = dict(
        ...
        # jerasure-specific
        w=dict(type='str', required=False),
        packetsize=dict(type='str', required=False),
        jerasure_per_chunk_alignment=dict(type='bool', required=False),
        # lrc-specific (low-level form)
        crush_locality=dict(type='str', required=False),
        mapping=dict(type='str', required=False),
        layers=dict(type='str', required=False),
        crush_steps=dict(type='str', required=False),
        # common CRUSH / loader
        ...
        crush_num_failure_domains=dict(type='str', required=False),
        crush_osds_per_failure_domain=dict(type='str', required=False),
    ...
    if state == "present":
        ...
        changed = not profile_satisfied(user_profile, current_profile)
        if changed:
