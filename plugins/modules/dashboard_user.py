...
# ---------------------------------------------------------------------------
# Vendored from:
#   https://github.com/ceph/ceph-ansible/blob/8599b192d33e1c55104c1c2fd1abfa8431c64664/library/ceph_dashboard_user.py
...
#   - state=present: check the return code of set_roles/set_password and fatal()
#     on failure (upstream ignored it, so a policy-rejected password failed
#     silently and reported ok). The password still cannot be diffed (the
#     dashboard only exposes a hash), so it is re-applied every run.
...
from ansible_collections.vexxhost.ceph.plugins.module_utils.common import generate_cmd, is_containerized, exec_command, exit_module, fatal
        ...
        if rc == 0:
            ...
            if user['roles'] != roles:
                ...
                if rc != 0:
                    fatal(err, module)
                changed = True
            # The dashboard never exposes the stored password, so it cannot be
            # diffed: re-apply it every run and surface any policy rejection
            # (weak password, blocklisted, etc.) instead of swallowing it.
